...
class PCMonitor:
    ...
    def find_esp32(self):
        ...
        for port in ports:
            # ESP32-S3 USB Serial JTAG zeigt sich als:
            # - Englisch: "USB Serial Device"
            # - Deutsch: "Serielles USB-Gerät"
            desc_upper = port.description.upper()

            # Prüfe auf englische oder deutsche Bezeichnung
            is_usb_serial = (
                ("USB" in desc_upper and "SERIAL" in desc_upper) or  # Englisch
                ("USB" in desc_upper and "SERIELL" in desc_upper) or  # Deutsch
                ("SERIELLES USB" in desc_upper)  # Deutsch vollständig
            )
